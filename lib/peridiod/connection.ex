defmodule Peridiod.Connection do
  ...
  [peridio.com](https://www.peridio.com) channel changes.
  ...
  [peridio.com](https://www.peridio.com) as part of the overall health
  ...
  :heart.set_callback(Peridiod.Connection, :check!)
  ```
  ...
  defmodule MyApp.Checker do
    def health_check do
      with :ok <- Peridiod.Connection.check,
  ...
  ```
  # 60 second timeout
  config :peridiod, connection_timeout: 60
  ```
  """
  @spec check() :: :ok | {:error, {:disconnected_too_long, integer()}}
  def check() do
    timeout = Application.get_env(:peridiod, :connection_timeout, 900)
    now = current_time()
  ...
  def check!() do
    unless check() == :ok do
      raise "too much time has passed since a successful connection to Peridio"
    end

defmodule Peridiod.Config do
  ...
  defp build_config(%Config{} = config, config_file) do
    ...
    config =
      ...
      |> override_if_set(:shadow_executable, get_in(config_file, ["shadow", "executable"]))
      |> override_if_set(:shadow_args, get_in(config_file, ["shadow", "args"]))
      |> override_if_set(:shadow_enabled, get_in(config_file, ["shadow", "enabled"]))

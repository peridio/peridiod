defmodule Peridiod.Binary.Downloader do
  ...
  def handle_response(
    ...
    # This is likely a range request response - log the details
    new_content_length = fetch_content_length(headers)

    if new_content_length != content_length do
      Logger.info(
        "[Stream Downloader #{state.id}] Range response received - Server returned #{new_content_length} bytes (partial content)"
      )
    end
  ...
  defp resume_download(
    ...
    # Determine if this is a range request or initial download
    download_type =
    ...
    Logger.info(
      "[Stream Downloader #{state.id}] #{download_type} download attempt #{state.retry_number} #{uri}"
    )
  ...
  defp add_range_header(headers, %Downloader{downloaded_length: dl} = state) when dl > 0 do
    # Use optional range-end format: bytes=start- to request from start position to end of file
    range_header = "bytes=#{dl}-"

    Logger.info(
      "[Stream Downloader #{state.id}] Range request - Header: '#{range_header}', Resuming download from #{dl} bytes"
    )

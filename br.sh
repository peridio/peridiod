...
if [[ "$RUN" == true ]]; then
  ...
  docker run -it \
    --name "$CONTAINER_NAME" \
    --network host \
    --add-host="device.test.peridio.com:192.168.0.104" \
    -v "$(pwd)/support/peridio.json:/etc/peridiod/peridio.json" \
    -v "/some/path/to/server-root-ca.pem:/etc/peridiod/ca-cert.pem:ro" \
    -v "$VOLUME_NAME:/var/lib/peridiod" \
    -e PERIDIO_PRIVATE_KEY="$(base64 < /some/path/to/device-1234-private-key.pem)" \
    -e PERIDIO_CERTIFICATE="$(base64 < /some/path/to/device-1234-certificate.pem)" \
    peridiod
fi
